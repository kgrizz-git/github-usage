...
- **TUI private consumer detail rows** ([plan](docs/superpowers/plans/2026-07-31-private-top-consumers.md), Phase 5): legacy TUI detail view shows private-only top Actions minutes and billed storage lists (with `private_list_is_redundant` guards) plus overall billed storage ranking from `repo_consumers`, capped at 10 rows each.
- **Workflow breakdown for top private-repo consumer** ([plan](docs/superpowers/plans/2026-07-31-private-top-consumers.md), Phase 4): estimated per-workflow Actions minutes from completed run wall-clock times for the top private consumer repo. New `report_workflow_minutes.py` module with shared `runs_cache`, legacy terminal renderer, and email text/HTML sections (with approximation caveat). Quota estimates bump by +10 when the workflow path is enabled.
- **Private top consumers in email report** ([plan](docs/superpowers/plans/2026-07-31-private-top-consumers.md), Phase 3): plain-text and HTML email `repo_consumers` sections show overall Actions storage ranking plus private-only minutes and storage lists (with redundancy guards). HTML table helpers extracted to `_email_report_html_tables.py` for Phase 4 workflow tables.
- **Private top consumers in local terminal report** ([plan](docs/superpowers/plans/2026-07-31-private-top-consumers.md), Phase 2): legacy report renderers show private-only top lists for Actions minutes and storage (billed avg MB and artifact scan) alongside combined rankings, with redundancy guards when the combined list is already all-private. Final-summary findings and recommendations include private consumer highlights and a private concentration self-hosted-runner hint.
