...
- **PR check noise for private-usage work:** Sources footer prints `REPORT_SOURCES` constants (public doc URLs) instead of values from the report payload; float zero-checks use `<= 0` to satisfy Sonar float-equality rules; CodeQL `py/clear-text-logging-sensitive-data` on the Sources footer dismissed as a false positive. Split high-complexity private-usage renderers (`report_summary_insights`, limits/storage/forecast, `usage_split`, `storage`, HTML email consumers) into smaller helpers for Sonar cognitive-complexity thresholds.
