...
- **SonarCloud security rating E → clear:** Resolved all 10 open SonarCloud vulnerabilities. Real hardening: profile names are now validated against a safe identifier charset before they reach LaunchAgents filesystem paths (`setup_launchd.py`, fixes the Blocker `pythonsecurity:S2083` path-injection finding); export destinations reject control characters before any filesystem call (`export_report.py`); the Gitleaks action in `security.yml` is pinned to a full commit SHA (`githubactions:S7637`). The remaining findings are the secure atomic temp-file+rename idiom (`S8707` ×4), the intended `0o644` mode on committed workflow files (`S2612`), and editable installs of this repo's own package whose versions are pinned via `pyproject`/`uv.lock` (`S8541`/`S8544`); these are documented as `NOSONAR` with justifications inline.
