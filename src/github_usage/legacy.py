#!/usr/bin/env python3
"""
github-usage-v3.py — GitHub Monthly Usage Report v3
...
Usage:
    ./github-usage-v3.py [GITHUB_TOKEN]
    python3 github-usage-v3.py [GITHUB_TOKEN]
...
class GitHubAPI:
    ...
    def get_all_pages(self, path, params=None):
        ...
        per_page = 100
        while True:
            result = self.request("GET", path, {**(params or {}), "page": page, "per_page": per_page})
            if not result:
                break
...
def get_billing_summary(api, username, product):
    ...
    try:
        billing = api.request("GET", f"/users/{username}/settings/billing/usage/summary", {"product": product})
    except RuntimeError:
        return None
    if not billing:
        return None
...
def get_premium_request_usage(api, username, product="copilot", model=None):
    ...
    if model:
        params["model"] = model
    try:
        data = api.request("GET", f"/users/{username}/settings/billing/premium_request/usage", params if params else None)
    except RuntimeError:
        return None
    if not data:
        return None
    ...
    for item in items:
        ...
        if m not in by_model:
            by_model[m] = {"items": [], "total_requests": 0, "total_gross": 0, "total_discount": 0, "total_net": 0}
...
def get_actions_per_repo(api, owner, repo):
    ...
    try:
        billing = api.request("GET", f"/users/{owner}/settings/billing/usage/summary", {
            "product": "Actions",
...
def show_rate_limits(api):
    ...
    # Standard limits
    print()
    for name, key in [("Core API", "core"), ("GraphQL API", "graphql"),
                       ("Search API", "search"), ("Code Scanning", "code_scanning_upload")]:
        r = resources.get(key, {})
        rem = r.get("remaining", "?")
        lim = r.get("limit", "?")
        used = r.get("used", 0)
        ...
        reset_str = ""
        if reset_ts:
            reset_str = f"  (resets {datetime.fromtimestamp(reset_ts, tz=timezone.utc).strftime('%H:%M UTC')})"
...
def show_actions_summary(api, username, user_minutes, user_storage_gb_hours, sku_breakdown):
    ...
    for sku, item in sku_breakdown.items():
        ...
        print(f"    {sku:<30} {qty:>10.4f} {unit:<18} {fmt_price(gross):>10} {fmt_price(discount):>10} {fmt_price(net):>10}")
    print()
...
def show_actions_per_repo(api, repos):
    ...
    for repo in repos:
        ...
        print(f"  {full:<45} {minutes:>10.1f} {storage_gb_hours:>10.4f} {avg_mb:>10.1f} {fmt_price(gross):>10}")
    ...
    print(f"  {'-'*45} {'-'*10} {'-'*10} {'-'*10} {'-'*10}")
    print(f"  {'TOTAL':<45} {total_mins:>10.1f} {total_gb:>10.4f} {total_mb:>10.1f} {fmt_price(total_gross):>10}")
    print()
    return repo_data
...
def show_actions_top_consumers(repo_data):
    ...
    for full, minutes, _, avg_mb, gross, _ in sorted_repos[:10]:
...
def show_copilot_summary(api, username):
    ...
    for sku, item in items.items():
        ...
        total_net += net
        print(f"    {sku:<35} {qty:>10.2f} {unit:<10} @ {fmt_price(price)}/ea | gross: {fmt_price(gross)} | discount: {fmt_price(discount)} | net: {fmt_price(net)}")
    print()
    print(f"    {'TOTAL':<35} {total_requests:>10.2f} requests | gross: {fmt_price(total_gross)} | discount: {fmt_price(total_discount)} | net: {fmt_price(total_net)}")
    print()
    ...
    for model, data in sorted(premium_by_model.items(), key=lambda x: x[1]["total_requests"], reverse=True):
        print(f"    {model}:")
        print(f"      Total requests: {data['total_requests']:.2f}")
        print(f"      Gross: {fmt_price(data['total_gross'])} | Discount: {fmt_price(data['total_discount'])} | Net: {fmt_price(data['total_net'])}")
...
def show_full_billing_history(api, username):
    ...
    agg = defaultdict(lambda: {"entries": 0, "total_qty": 0, "total_gross": 0, "total_discount": 0, "total_net": 0, "units": set(), "repos": set(), "months": set()})
    ...
    print(f"  {'PRODUCT/SKU':<40} {'ENRIES':>6} {'TOTAL QTY':>12} {'UNITS':<12} {'GROSS':>10} {'DISCOUNT':>10} {'NET':>10}")
    ...
    for key in sorted(agg.keys()):
        ...
        print(f"  {key:<40} {d['entries']:>6} {qty_str:>12} {units:<12} {fmt_price(d['total_gross']):>10} {fmt_price(d['total_discount']):>10} {fmt_price(d['total_net']):>10}")
        if repos:
            print(f"    repos: {repos}")
        if months:
    ...
    print(f"  {'-'*40} {'-'*6} {'-'*12} {'-'*12} {'-'*10} {'-'*10} {'-'*10}")
    print(f"  {'TOTAL':<40} {'':>6} {'':>12} {'':>12} {fmt_price(total_gross):>10} {fmt_price(total_discount):>10} {fmt_price(total_net):>10}")
    print()
...
def show_monthly_costs(repo_data, username, api):
    ...
    print(f"  {'-'*25} {'-'*12} {'-'*12} {'-'*12}")
    print(f"  {'Actions':<25} {fmt_price(actions_gross):>12} {fmt_price(actions_discount):>12} {fmt_price(actions_net):>12}")
    print(f"  {'Copilot':<25} {fmt_price(copilot_gross):>12} {fmt_price(copilot_discount):>12} {fmt_price(copilot_net):>12}")
    print(f"  {'Git LFS':<25} {fmt_price(lfs_gross):>12} {fmt_price(lfs_discount):>12} {fmt_price(lfs_net):>12}")
    print(f"  {'-'*25} {'-'*12} {'-'*12} {'-'*12}")
    print(f"  {'TOTAL':<25} {fmt_price(total_gross):>12} {fmt_price(total_discount):>12} {fmt_price(total_net):>12}")
    print()

    if total_discount > 0:
        savings_pct = (total_discount / total_gross * 100) if total_gross > 0 else 0
        print(f"  You're saving {fmt_price(total_discount)} ({savings_pct:.1f}% discount) this month!")
        print()
...
def show_limits_summary(username, user_minutes, user_storage_gb_hours):
    ...
    # Copilot limits note
    print(f"  Copilot Pro:")
    print(f"    Includes: Copilot Chat, Copilot Agent, Code Review, etc.")
    print(f"    Premium requests are billed at $0.04/request after included allowance.")
    print(f"    (Check your plan details for exact premium request limits)")
    print()

def show_what_else(api, username):
    """Show info about other available data points."""
    print_section("Other Available Data Points")
    print(f"  Products available via billing API:")
    print(f"    - actions        : GitHub Actions compute & storage")
    print(f"    - copilot        : Copilot Chat, Agent, Code Review, etc.")
    print(f"    - git_lfs        : Git LFS storage")
    print(f"    - codespaces     : GitHub Codespaces compute & storage")
    print(f"    - packages       : GitHub Packages (npm, Docker, etc.)")
    print(f"    - models         : GitHub Models (LLM API usage)")
    print()
    print(f"  Other endpoints available:")
    print(f"    - /users/{username}/settings/billing/usage")
    print(f"      Full billing history (all products, daily granularity)")
    print(f"    - /users/{username}/settings/billing/premium_request/usage")
    print(f"      Premium request usage by model (Claude Sonnet 4.6, GPT-5.4, etc.)")
    print()
    print(f"  Rate limits tracked:")
    print(f"    - Core API:       5000/hour (authenticated)")
    print(f"    - GraphQL API:    5000/hour (authenticated)")
    print(f"    - Search API:     30/minute (full text search)")
    print(f"    - Code Scanning:  5000/hour")
    print(f"    - Actions Runner: 10000/hour (registration)")
    print(f"    - SCIM:           15000/hour (enterprise)")
    print(f"    - Audit Log:      1750/hour")
    print(f"    - Dependency:     100/hour (snapshots + SBOM)")
    print()
...
def show_base_costs(api, username, actions_sku, copilot_summary, lfs_summary):
    ...
    if not actions_minutes_found:
        print(f"    No compute minutes billed.")
    print(f"    Standard tier: ~$0.008/min (Linux), ~$0.016/min (Windows), ~$0.016/min (macOS)")
    print(f"    Free tier: 2,000 min/month for personal repos")
    print()

    print(f"  Actions Storage:")
    ...
    for sku, item in (actions_sku or {}).items():
        ...
        if unit == "gigabyte-hours":
            ...
            print(f"    {sku:<40} {fmt_price(price)}/GB-hr  × {qty:.2f} GB-hrs ({avg_mb:.0f} MB avg)  = {fmt_price(net)}")
            actions_storage_found = True
    if not actions_storage_found:
        print(f"    No storage billed.")
    print(f"    Standard: ~$0.01/GB-month")
    print(f"    Free tier: 500 MB for personal repos")
    print()

    print(f"  Copilot Premium Requests:")
    copilot_found = False
    if copilot_summary and copilot_summary["items"]:
        ...
        for sku, item in copilot_summary["items"].items():
            ...
            qty = item.get("grossQuantity", 0)
            if price > 0:
                print(f"    {sku:<40} {fmt_price(price)}/req  × {qty:.0f} reqs  = {fmt_price(item.get('netAmount', 0))}")
                copilot_found = True
                all_prices.add(price)
        if all_prices:
            print(f"    Base rate: {max(all_prices):.4f}/req (highest observed)")
    if not copilot_found:
        print(f"    No premium requests billed.")
    print(f"    Copilot Pro: ~$0.04-0.08/request for premium features")
    print()
    ...
    if lfs_summary and lfs_summary["items"]:
        for sku, item in lfs_summary["items"].items():
            ...
            qty = item.get("grossQuantity", 0)
            if price > 0:
                print(f"    {sku:<40} {fmt_price(price)}/GB  × {qty:.2f} GB  = {fmt_price(item.get('netAmount', 0))}")
                lfs_found = True
    if not lfs_found:
        print(f"    No LFS storage billed.")
    print(f"    Standard: ~$1/GB-month after 1 GB free")
    print()


def show_final_summary(username, user_minutes, user_storage_gb_hours, actions_gross,
                       actions_discount, actions_net, repo_data,
                       copilot_summary, lfs_summary, storage_analysis, api):
    ...
    print(f"    Total Discount:  {fmt_price(total_discount):>12}  ({total_discount/total_gross*100:.1f}% off)")
    print(f"    Total Net:       {fmt_price(total_net):>12}")
    print()

    # ── 2. Biggest Consumers by Category ──
    print(f"  2. BIGGEST CONSUMERS BY CATEGORY")
    print(f"  {'─' * 55}")
    ...
    for full, mins, gb, avg_mb, gross, _ in sorted_repos[:5]:
        ...
        for model, data in sorted(premium_by_model.items(), key=lambda x: x[1]["total_requests"], reverse=True):
            ...
            for item in data["items"]:
                ...
                if pp > 0:
                    price = pp
                    break
            print(f"      {model:<30} {data['total_requests']:>10.0f} reqs  @ {fmt_price(price)}/req  = {fmt_price(data['total_net'])}")
    else:
        print(f"      No model-level data available.")
    print()

    # Git LFS
    if lfs_summary and lfs_summary["items"]:
        print(f"    Git LFS Storage:")
        for sku, item in lfs_summary["items"].items():
            qty = item.get("grossQuantity", 0)
            unit = item.get("unitType", "")
            price = item.get("pricePerUnit", 0)
            net = item.get("netAmount", 0)
            print(f"      {sku:<30} {qty:>12.4f} {unit:<10} @ {fmt_price(price)}/ea  = {fmt_price(net)}")
    else:
        print(f"    Git LFS: No usage found.")
    print()

    # ── 3. Storage Breakdown by Repo (Artifacts, Releases, LFS) ──
    print(f"  3. STORAGE BREAKDOWN BY REPOSITORY")
    print(f"  {'─' * 55}")

    sorted_by_storage = sorted(storage_analysis.get("repos", []), key=lambda x: x["total_storage"], reverse=True)
    if sorted_by_storage:
        ...
        if sorted_by_storage:
            ...
            print(f"    Top storage consumer: {top_storage['name']} ({fmt_price(top_storage['total_storage'])})")
            print(f"    Breakdown:")
            for item in top_storage.get("items", []):
                print(f"      {item['type']:<20} {item['count']:>5} items  {fmt_price(item['storage'])}  ({item['size']})")
            print()
    else:
        print(f"    No storage data available from repositories.")
        print()

    # ── 4. Resource Utilization vs Limits ──
    print(f"  4. RESOURCE UTILIZATION vs LIMITS")
    print(f"  {'─' * 55}")
    ...
    if min_pct > 80:
        print(f"    ⚠ HIGH USAGE — approaching free tier limit!")
    elif min_pct > 50:
        print(f"    → Moderate usage — on track to use half your free allowance")
    print()
    ...
    print(f"    Actions Storage:     {avg_storage_mb:>8.1f} / {free_storage_mb} MB ({storage_pct:.1f}% of free tier)")
    ...
    print(f"  5. TOP 3 MOST IMPACTFUL FINDINGS")
    print(f"  {'─' * 55}")
    ...
    # ── 6. Recommendations ──
    print(f"  6. QUICK RECOMMENDATIONS")
    print(f"  {'─' * 55}")
    recs = []
    if min_pct > 80:
        recs.append("Upgrade from free tier or optimize Actions workflows — you're near your minute limit.")
    if sorted_repos and len(sorted_repos) > 1:
        ...
        if top2_sum / user_minutes * 100 > 70 if user_minutes else False:
            recs.append(f"Top 2 repos consume {top2_sum/user_minutes*100:.0f}% of Actions — consider self-hosted runners to save.")
    if premium_by_model:
        models = list(premium_by_model.keys())
        if len(models) > 2:
            recs.append(f"Using {len(models)} Copilot models — consolidate to reduce cost complexity.")
    ...
    if sorted_by_storage:
        ...
        if release_assets:
            ...
            if total_release_size > 100:
                recs.append(f"Release assets in {top_st['name']} use {fmt_price(total_release_size)} — consider using GitHub Pages or external storage for large binaries.")
    if not recs:
...
def main():
    ...
    if not token:
        ...
        print("  Usage: ./github-usage-v3 <token>")
    ...
    try:
        ...
        show_final_summary(username, user_minutes, user_storage_gb, actions_gross_total,
                           actions_discount_total, actions_net_total, repo_data,
                           copilot_summary, lfs_summary, storage_analysis, api)
